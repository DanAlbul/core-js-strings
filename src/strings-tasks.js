...
function removeFirstOccurrences(str, value) {
  return str.indexOf(value) === -1 ? str : str.replace(value, '');
}
...
function removeLastOccurrences(str, value) {
  ...
  const start = str.slice(0, str.lastIndexOf(value));
  const end = str.split(value).pop();
  return `${start}${end}`;
}
...
function isPalindrome(str) {
  ...
  const purePalindrome = pureString.split('').reverse().join('');
  return pureString === purePalindrome;
}
...
function extractNameFromTemplate(value) {
  ...
  return `${splitted[1]} ${splitted[2].slice(0, -1)}`;
}
...
function encodeToRot13(str) {
  return str.replace(/[a-z]/gi, (line) =>
    String.fromCharCode(
      line.charCodeAt(0) + (line.toLowerCase() <= 'm' ? 13 : -13)
    )
  );
}
